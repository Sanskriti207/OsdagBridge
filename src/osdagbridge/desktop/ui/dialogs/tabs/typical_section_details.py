...
class TypicalSectionDetailsTab(QWidget):
    ...
    def get_cad_data(self):
        """
        Data required for CROSS SECTION & TOP VIEW CAD
        """
        return {
            "carriageway_width": float(self.carriageway_width),
            "girder_count": self.get_girder_count(),
            "girder_spacing": self.get_girder_spacing(),
            "deck_overhang": self.get_deck_overhang(),
            "deck_thickness": self.get_deck_thickness(),
            "footpath_width": self.get_footpath_width(),
            "footpath_thickness": self.get_footpath_thickness(),
            "footpath_config": self.get_footpath_config(),
            "median_present": self.is_median_present(),
        }
    ...
    def get_girder_count(self):
        try:
            return int(self.no_of_girders.text())
        except Exception:
            return 2


    def get_girder_spacing(self):
        try:
            return float(self.girder_spacing.text())
        except Exception:
            return DEFAULT_GIRDER_SPACING


    def get_deck_overhang(self):
        try:
            return float(self.deck_overhang.text())
        except Exception:
            return 0.35 * DEFAULT_GIRDER_SPACING


    def get_deck_thickness(self):
        try:
            return float(self.deck_thickness.text())
        except Exception:
            return 200.0
    ...
    def get_skew_angle(self):
        ...
        Defaults to 0.0 if skew input is not present.
        """
        if hasattr(self, "skew_input") and self.skew_input.text():
            try:
                return float(self.skew_input.text())
            except ValueError:
                return 0.0
        return 0.0


    def get_footpath_width(self):
        if self.footpath_value == "None":
            return 0.0
        try:
            return float(self.footpath_width.text())
        except Exception:
            return 1.5


    def get_footpath_thickness(self):
        if self.footpath_value == "None":
            return 0.0
        try:
            return float(self.footpath_thickness.text())
        except Exception:
            return 200.0


    def get_footpath_config(self):
        return self.footpath_value


    def is_median_present(self):
        return self.median_tab.isVisible() and self.median_tab.isEnabled()
